/*
 * In-page feedback — drop-in review library for static HTML plans.
 *
...
(function () {
  ...
  function renderPending() {
    ...
    // Show a "processing…" banner while we wait for the agent to
      ...
      if (isBatchStale) {
        banner.innerHTML = `
          ...
          <div class="cf-processing-status">
            Your batch is saved in <code>feedback/inbox.jsonl</code>. The agent watching this session should process it soon. If nothing happens, ask <em>"process my feedback"</em>.
          </div>
  ...
  async function submitBatch() {
    ...
    try {
      ...
      syncTitle();
      renderPending();
      showToast("batch sent — agent is processing", 3500);
